...
#筛选出用户自己的函数
def isUserFunc(addr):
    functionName = Name(GetFunctionAttr(addr,FUNCATTR_START))
...
def FirstWriteRegOp( addr,reg,op,Optype = None):
    addr = FindCode(addr,SEARCH_NEXT)
    while (addr != BADADDR):
        #第一个操作数是指定寄存器
...
def setEnumAndCom(addr):
    offset = GetOperandValue(addr,1)
    if (offset / 4) < len(JNINativeInterface) and (offset /4) >= 4:
        comment = JNINativeInterface[offset / 4]
        #添加注释
        MakeRptCmt(addr,comment)
        #设为枚举类型
        OpEnumEx(addr,1,enum_jni_id,0)
        print 'set comment and emun 0x%x' % addr
...
def find_LDR(addr, reg):
    ...
    while (addr != BADADDR):
        ...
        if GetMnem(addr) == 'LDR' and GetOpnd(addr,0) == reg:
            #print GetOpnd(addr,1),GetOpType(addr,1)

            #右侧的寄存器列表
            regs = filter(lambda x: x.startswith('R') , GetOpnd(addr,1)[1:-1].split(','))
            ...
            # LDR RX,[RX,#0xXX]
            if GetOpType(addr,1) == OP_Base_Index_Displacement:
                ...
                if FirstWriteRegOp( addr, regs[0],'LDR',OP_Base_Index_Displacement):
                    ...
                    off = GetOperandValue(addr, 1)

                    if len(regs) == 1:
                        #LDR RX,[RY + 0xXX]
                        #单个寄存器
                        wRegaddr = writeRegAddr(addr,regs[0])
                        if GetMnem(wRegaddr) == 'ADD' and  FirstWriteRegOp(wRegaddr, regs[0],'LDR',OP_Base_Index):
                            off2 = GetOperandValue(wRegaddr, 1)
                            offset = off + off2
                            if off != 0:
            ...
            #EXP: LDR R3,[R1,R2]
            elif GetOpType(addr,1) == OP_Base_Index: 
    ...
    while (addr > start):
        ...
        if GetMnem(addr) == "BLX":
            ...
            if Opnd0 in COMMON_REG:
                #print '%x %s %s'% (addr,GetMnem(addr),GetOpnd(addr,0))
                find_LDR(addr, Opnd0)
        pass
...
def enum_user_function():
    ...
    while (addr != BADADDR):
        function_start = addr
        function_end = GetFunctionAttr(addr,FUNCATTR_END)
        #过滤出用户的函数
        if isUserFunc(addr):
            #尝试校正
...
def init_enum(enum_name, fields_list , offset ,comments):
    ret_id = AddEnum(-1,enum_name,0x1100000);
    synthesis = list()
    ...
    if ret_id != 0xFFFFFFFF:
        for fun,com in synthesis:
            AddConstEx( ret_id , fun , offset , -1);
            SetConstCmt(GetConstEx(ret_id,offset,0,-1),com ,1);
            offset += 4
    return ret_id

#2014/09/10 10:20 
#获取枚举
enum_jni_id = GetEnum(Enum_JNI_Name)
...
elif GetEnumSize(enum_jni_id) != len(jni_names_list):
    #成员个数不对，重新创建
